			...
			if($size > 2097152){
				...
				if(move_uploaded_file($tmp,$path)) {
					...
					if(mysqli_num_rows($check) > 0){
						$save = $this->db->query("UPDATE student set studentname='".$fname."',gender='".$gender."',school_id='".$smcid."',paddress='".$paddress."',birthdate='".$bday."',civilstatus='".$civilstat."',religion='".$religion."',contact='".$cphone."',gradelevel='".$Gradelevel."',strand='".$strand."',mothersname='".$mother."',fathersname='".$father."',parents_address='".$pa_addr."',parents_contact='".$pa_cont."',nupload='".$path."' WHERE id = $loginid");
					}
					else{
						$save = $this->db->query("INSERT INTO student (id,userid,studentname,gender,school_id,paddress,birthdate,civilstatus,religion,contact,gradelevel,strand,mothersname,fathersname,parents_address,parents_contact,nupload) VALUES ((SELECT id FROM users where id='".$loginid."'),(SELECT id FROM users where id='".$loginid."'),'".$fname."','".$gender."','".$smcid."','".$paddress."','".$bday."','".$civilstat."','".$religion."','".$cphone."','".$Gradelevel."','".$strand."','".$mother."','".$father."','".$pa_addr."','".$pa_cont."','".$path."') ");
					}
					if($save)
						return 1;
